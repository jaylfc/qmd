...
class LLMSession implements ILLMSession {
  ...
  async embedBatch(texts: string[], options?: EmbedOptions): Promise<(EmbeddingResult | null)[]> {
    // MERGE NOTE (jaylfc fork, v2.5.x update): upstream routes batch embed
    // through getLlamaCpp() (local llama.cpp only). This fork keeps it on the
    // abstract getLLM() so a remote / NPU backend (RemoteLLM) stays in the
    // embed path — that abstraction is the whole point of this fork. The new
    // `options` arg is threaded through; RemoteLLM ignores it harmlessly.
