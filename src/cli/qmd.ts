...
// MERGE NOTE: union of both import sets — keep our remote-server imports
// (setDefaultLLM, RemoteLLM, startServer, DEFAULT_*_MODEL_URI) AND upstream's
// new ones (setDefaultLlamaCpp, LlamaCpp, resolveModels, inspectGgufFile, ...).
import { disposeDefaultLlamaCpp, getDefaultLlamaCpp, setDefaultLLM, setDefaultLlamaCpp, LlamaCpp, withLLMSession, pullModels, DEFAULT_EMBED_MODEL_URI, DEFAULT_GENERATE_MODEL_URI, DEFAULT_RERANK_MODEL_URI, DEFAULT_MODEL_CACHE_DIR, resolveEmbedModel, resolveGenerateModel, resolveRerankModel, resolveModels, inspectGgufFile, isDarwinMetalMitigationActive } from "../llm.js";
...
async function showStatus(): Promise<void> {
  ...
  // MERGE NOTE: upstream removed this block from `status`; kept ours because
  // it carries the remote-server (QMD_SERVER) awareness this fork needs. Pure
  // status-output cosmetics — safe to drop later if it duplicates upstream.
  try {
...
if (isMain) {
  ...
  switch (cli.command) {
    ...
    // MERGE NOTE: kept BOTH cases — our `serve` (the fork's remote-server
    // feature) and upstream's new `bench`. They were tangled into one region
    // by the merge; reconstructed as two complete switch cases.
    case "serve": {
