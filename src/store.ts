  ...
  const result = await withLLMSessionForLlm(llm, async (session) => {
    ...
    for (const batchMeta of batches) {
      ...
      for (let batchStart = 0; batchStart < batchChunks.length; batchStart += BATCH_SIZE) {
        ...
        // MERGE NOTE: took upstream. Our fork's old 99%/BATCH_SIZE*4 generosity
        // was compensating for a weak hand-rolled 3x retry; upstream now has a
        // proper retry queue (recordFailure/retryFailedChunks/MAX_RETRY_ATTEMPTS
        // + force mode), so the 80% abort only trips after real retry exhaustion.
        ...
        try {
          ...
          if (!session.isValid) {
            ...
            for (const chunk of chunkBatch) {
              // MERGE NOTE: took upstream. tryEmbedChunk + retryFailedChunks
              // (queue-based, capped, force-mode) supersedes our old manual
              // 3x loop and is strictly more resilient for SBC/NPU. Our loop
              // also used the old 2-arg formatDocForEmbedding and wouldn't
              // compile against upstream's 3-arg signature.
              await tryEmbedChunk(chunk);
              batchChunkBytesProcessed += chunk.bytes;
              await retryFailedChunks();
            }
